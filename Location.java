...
public abstract class Location implements AbleToLocate {
    ...
    private final List<Locatable> locatedGroups = new LinkedList<>();
    ...
    public List<Room> getRooms(Locatable group) {
    ...
    public List<Locatable> getLocatedGroups() {
        return locatedGroups;
    }

    public boolean hasLocatedGroup(Locatable group) {
    ...
    public void locate(Locatable group) {
        if (hasLocatedGroup(group)) {
        ...
        System.out.println(group + " - теперь находится в - " + this + "\n");
    }
    ...
    private void addLocatedGroup(Locatable group) {
        group.setLocation(this);
        locatedGroups.add(group);
    }

    public void dislocate(Locatable group) {
        if (!hasLocatedGroup(group)) {
            throw new IllegalArgumentException("В локации - " + name + " - нет данной группы коротышек");
        }
        ...
        System.out.println(group + " - теперь не находится в - " + this + "\n");
    }
    ...
    private void removeLocatedGroup(Locatable group) {
